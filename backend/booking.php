						...
						<table class="table table-sm table-hover table-striped table-bordered table-light" id="table-admin">
							...
							<tbody>
								<?php while($get_for_booking_results = mysqli_fetch_assoc($get_for_booking_transactions)){
									...
									preg_match_all('/Booking Animal For Transport-(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $get_date_data_results['other_transaction_dates'], $matches);
							...
							<div class="modal-body" style="padding-bottom: 0px;">
								<div class="row form-modal" style="padding-right: 20px;">
									...
									<div class="modal-footer popup-footer">
										...
										<button type="submit" id="shippingAttachmentsSubmit" class="btn action-view" name="insertBookingAttachments" onclick="uploadTransportAttachments(event)">Proceed</button>
									</div>
								</div>
							</div>
						</form>
