...
<body>
	...
	<section id="content">
		...
		<main>
			<!-- Page Header -->
			<h1 class="title">Final Payment</h1>
			<!-- Page Header -->
			...
			<ul class="breadcrumbs">
				...
				<li class="divider">/</li>
				<li><a href="#" class="active">Final Payment</a></li>
			</ul>
			<!-- Breadcrumbs -->
					...
					<div class="table-responsive">
						<table class="table table-sm table-hover table-striped table-bordered table-light" id="table-admin">
							...
							<?php while($get_pending_final_payment_data_result =  mysqli_fetch_array($get_pending_final_payment_data)){
									...
									preg_match_all('/Settle Remaining Payment-(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $get_date_data_results['other_transaction_dates'], $matches);
					...
					<table class="table table-sm table-hover table-striped table-bordered table-light" id="table-admin">
							...
							<?php while($get_successful_final_payment_data_result =  mysqli_fetch_array($get_successful_final_payment_data)){
								...
								<tr>
									...
									<td>
										<button class="btn-sm btn m-1 table-action-btn action-view" data-toggle="modal" data-target="#viewClientRequest" data-transaction-id="<?php echo $transactionID; ?>" onclick="viewClientRequest(this);"><i class="material-icons table-action-icon">visibility</i></button>
										<button class="btn-sm btn m-1 table-action-btn action-approve" onclick="approveFinalPayment('<?php echo $clientID ?>', '<?php echo $client_name ?>' , '<?php echo $transactionID; ?>')"><i class="material-icons table-action-icon">thumb_up</i></button>
										<button class="btn-sm btn m-1 table-action-btn action-approve" data-toggle="modal" data-target="#addPayment" data-client-id="<?php echo $clientID; ?>" data-transaction-id="<?php echo $transactionID; ?>" data-clientname="<?php echo $client_name; ?>" onclick="costClientDetails(this)"><i class="material-symbols-outlined table-action-icon">add_card</i></button>
										<button class="btn-sm btn m-1 table-action-btn action-deny" data-client-id="<?php echo $clientID; ?>" data-transaction-id="<?php echo $transactionID; ?>" data-clientname="<?php echo $client_name; ?>" onclick="rejectFinalPayment('<?php echo $clientID ?>', '<?php echo $client_name; ?>', '<?php echo $transactionID; ?>')"><i class="material-icons table-action-icon">thumb_down</i></button>
				...
				<div class="card-body table-card-body">
					...
					<div class="table-responsive">
						<table class="table table-sm table-hover table-striped table-bordered table-light" id="table-admin">
							...
							<tbody>
							<?php while($get_rejected_final_payments_result =  mysqli_fetch_array($get_rejected_final_payments)){
									$transactionID = $get_rejected_final_payments_result['transaction_id'];
									$clientID = $get_rejected_final_payments_result['client_id'];
									$dateID = $get_rejected_final_payments_result['date_id'];
									$animalID = $get_rejected_final_payments_result['animal_id'];
									$paymentID = $get_rejected_final_payments_result['payment_id'];
									$paymentType = $get_rejected_final_payments_result['payment_type'];
									$paymentMethod = $get_rejected_final_payments_result['payment_method'];
				...
				<div class="card-body table-card-body">
					...
					<div class="table-responsive">
						<table class="table table-sm table-hover table-striped table-bordered table-light" id="table-admin">
							...
							<tbody>
							<?php while($get_additional_final_payments_result =  mysqli_fetch_array($get_additional_final_payments)){
									$transactionID = $get_additional_final_payments_result['transaction_id'];
									$clientID = $get_additional_final_payments_result['client_id'];
									$dateID = $get_additional_final_payments_result['date_id'];
									$animalID = $get_additional_final_payments_result['animal_id'];
									$paymentID = $get_additional_final_payments_result['payment_id'];
									$paymentType = $get_additional_final_payments_result['payment_type'];
									$paymentMethod = $get_additional_final_payments_result['payment_method'];
			...
			<div class="modal fade" id="addPayment" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content popup">
						...
						<form id="addFinalPaymentCost" action="final_payment.php" method="POST" autocomplete="off" enctype="multipart/form-data">
								...
								<div class="row form-modal" style="padding-right: 20px;">
									<div class="col col-md-12 ml-auto">
										...
										<div class="row mb-3 ml-auto">
											...
											<input type="text" class="form-control" required pattern="[a-zA-Z0-9\s]+" oninput="validatePaymentAmountPattern(this)" placeholder="Payment Cost..." name="add_final_cost" id="add_final_cost">
											<input type="hidden" name="client_name" id="client_name">
											<input type="hidden" name="client_id" id="client_id">
											<input type="hidden" name="transaction_id" id="transaction_id">
										</div>
										<input type="hidden" id="addFinalPaymentInput" name="addFinalPayment">
									</div>
									<div class="modal-footer popup-footer">
										<button type="button" class="btn btn-secondary action-cancel" data-dismiss="modal">Close</button>
										<button type="submit" id="addFinalPaymentSubmit" class="btn action-view" name="addFinalPayment" onclick="addFinalPaymentValidate(event)">Approve Transaction</button>
									</div>
								</div>
							</div>
						</form>
			...
			<div class="modal fade" id="cancelwReturn" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content popup">
						...
						<form id="insertRFCwReturn" action="final_payment.php" method="POST" autocomplete="off" enctype="multipart/form-data">
							<input type="hidden" id="e_admin_id" name="e_admin_id">
							<div class="modal-body" style="padding-bottom: 0px;">
								<div class="row form-modal" style="padding-right: 20px;">
									<div class="col col-md-12 ml-auto">
