...
<body>
	...
	<section id="content">
		...
		<main>
			...
			<!-- Archived Animal Species -->
			<div class="card table-card">
				<div class="card-body table-card-body">
					<h4 class="card-title table-card-title">Archived Animal Species</h4>
					<p class="card-description table-card-description">
						Displaying all archived animal species entries from recently archived to earliest archived animal species.
					</p>
					<div class="table-search-action">
						<form action="#">
							<div class="form-group" style="flex: 95;">
								<input type="text" placeholder="Search" id="table-search-archived-species">
								<i class='bx bx-search icon'></i>
							</div>
						</form>
					</div>
					<div class="table-responsive">
						<table class="table table-sm table-hover table-striped table-bordered table-light" id="table-archived-species">
							<thead>
								<tr>
									<th>Species ID</th>
									<th>Submitted By</th>
							...
							<tbody>
								<?php while($get_archivedSpecies_result = mysqli_fetch_array($get_archivedSpecies)) { 
										$clientID = $get_archivedSpecies_result['submitted_by'];
								...
								?>
									<tr>
										<td><?php echo $get_archivedSpecies_result['species_id']; ?></td>
										...
										<td><?php echo $get_archivedSpecies_result['description']; ?></td>
										<td><?php echo $get_archivedSpecies_result['submitted_on']; ?></td>
										<td><?php echo $get_archivedSpecies_result['archived_on']; ?></td>
										<td>
											<button class="btn-sm btn m-1 table-action-btn action-view" onclick="retrieveSpecies('<?php echo $get_archivedSpecies_result['description']; ?>', '<?php echo $get_archivedSpecies_result['species_id']; ?>')"><i class="material-icons table-action-icon">keyboard_return</i></button>
											<button class="btn-sm btn m-1 table-action-btn action-deny" onclick="deleteSpecies('<?php echo $get_archivedSpecies_result['description']; ?>', '<?php echo $get_archivedSpecies_result['species_id']; ?>')"><i class="material-icons table-action-icon">delete</i></button>
										</td>
									</tr>
								<?php } ?>
							</tbody>
						</table>
						<?php if ($archivedSpecies_count == 0) { ?>
